...
- Settings sync: Keep your plugins and their settings synchronized between devices / apps (optional)
...
However, Discord is pretty indifferent about them, and there are no known cases of users getting banned for using client mods! So you should generally be fine as long as you don’t use any plugins that implement abusive behavior. But no worries, all inbuilt plugins are safe to use!
...
Additionally, make sure not to post screenshots with Vencord on a server where you might get banned for it.
</details>
