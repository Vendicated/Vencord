...
import { ApplicationCommandInputType, ApplicationCommandOptionType, ApplicationCommandType, Argument, ZCommand, CommandContext, Option } from "./types";

export * from "./commandHelpers";
export * from "./types";

export let BUILT_IN: ZCommand[];
export const commands = {} as Record<string, ZCommand>;
...
export const _init = function (cmds: ZCommand[]) {
    try {
        BUILT_IN = cmds;
...
export const _handleCommand = function (cmd: ZCommand, args: Argument[], ctx: CommandContext) {
 ...
 * @param opt
 */
export function prepareOption<O extends Option | ZCommand>(opt: O): O {
...
// investigate
function registerSubCommands(cmd: ZCommand, plugin: string) {
    cmd.options?.forEach(o => {
        ...
        registerZCommand(subCmd as any, plugin);
    });
}

export function registerZCommand<C extends ZCommand>(command: C, plugin: string) {
    if (!BUILT_IN) {
        console.warn(
            "[CommandsAPI]",
...
export function unregisterZCommand(name: string) {
