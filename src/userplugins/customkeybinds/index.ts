...
const SoundModule = findByProps("playSound") as { playSound?: (sound: string, volume?: number) => void; } | undefined;

const ToggleEventName = "vc-custom-keybinds-global-toggle";
const ErrorEventName = "vc-custom-keybinds-global-error";

function assertDesktopBridge() {
    if (!Native) {
...
function handleToggle(action: GlobalToggleAction) {
    ...
    SoundModule!.playSound(action, 0.5);
}
