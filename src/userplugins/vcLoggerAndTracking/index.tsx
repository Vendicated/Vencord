...
// const CONSOLE_LOG = /* console.CONSOLE_LOG = */ console.log;
...
function getContent({ channelId, oldChannelId, userId }: VoiceStateChangeEvent) {
    if (channelId !== oldChannelId) {
...
export default definePlugin({
    name: "VcLoggerAndTracking",
    description: "Announces and Logging when users join, leave, or move voice channels via narrator and in chat",
    authors: [{ name: "user undefined", id: 1234567890n }],
    ...
    flux: {
        VOICE_STATE_UPDATES({ voiceStates }: { voiceStates: VoiceStateChangeEvent[]; }) {
            ...
            for (const state of voiceStates) {
                ...
                const _user = UserStore.getUser(userId);

                console.log({ myGuildId, myChanId, myId, voiceStates, template, user, channel, displayName, nickname });

                if (content && settings.store.enableLogging) {
                    debugger;
                    var msg = sendBotMessage(
                        settings.store.logInCurrentVoiceChannel
                            ? myChanId
                            : SelectedChannelStore.getCurrentlySelectedChannelId(myGuildId as string) || myChanId,
                        { content, author: _user }
                    );
                    // msg
                }
