...
async function createQuoteImage(avatarUrl: string, name: string, quoteOld: string, grayScale: boolean): Promise<Blob> {
  let quote = removeCustomEmojis(quoteOld);
  ...
  function wrapText(
    ...
    for (let i = 0; i < words.length; i++) {
      let workSentence = preparingSentence.join(' ') + ' ' + words[i];
...
async function SendInChat(onClose) {
  ...
  let image = await createQuoteImage(sizeUpgrade(recentmessage.author.getAvatarURL()), recentmessage.author.username, recentmessage.content, grayscale);
  let preview = generateFileNamePreview(recentmessage.content);
  let imageName = `${preview} - ${recentmessage.author.username}`;
