...
import { GuildChannelStore, GuildMemberStore, Menu, React, RestAPI, UserStore } from "@webpack/common";
...
function sendPatch(channel: Channel, body: Record<string, any>, bypass = false) {
    ...
    Object.keys(usersVoice).forEach((key, index) => {
        ...
        const member = GuildMemberStore.getMember(channel.guild_id, userVoice.userId);
        const isBot = member?.user?.bot ?? false;

        // Skip if it's the current user or if skipBots is enabled and the user is a bot
        if ((bypass || userVoice.userId !== myId) && !(settings.store.skipBots && isBot)) {
...
const settings = definePluginSettings({
    ...
    skipBots: {
        ...
        description: "Skip bots when performing actions",
        default: true
    }
});
