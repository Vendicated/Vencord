...
export const settings = definePluginSettings({
    hideUnreads: {
        description: "Hide unread indicators for hidden channels.",
        type: OptionType.BOOLEAN,
        default: true,
        restartNeeded: true
    },
    showMode: {
        description: "How hidden channels should be displayed.",
        type: OptionType.SELECT,
        options: [
            { label: "Use normal channel style with a lock icon", value: ShowMode.LockIcon, default: true },
            { label: "Use muted channel style with a hidden-eye icon", value: ShowMode.HiddenIconWithMutedStyle },
        ],
        restartNeeded: true
    },
    defaultAllowedUsersAndRolesDropdownState: {
        description: "Whether the Allowed users and roles section should be expanded by default.",
        type: OptionType.BOOLEAN,
        default: true
    }
});
...
export default definePlugin({
    ...
    patches: [
        ...
        },
        {
            ...
            predicate: () => settings.store.showMode === ShowMode.LockIcon,
            replacement: {
                // Fallback lock icon in channel rows in case Discord changes icon switch internals
                match: /\.Children\.count.+?:null(?<=,channel:(\i).+?)/,
                replace: (m, channel) => `${m},$self.isHiddenChannel(${channel})?$self.HiddenChannelLockIcon():null`
            }
        },
        {
            find: "UNREAD_IMPORTANT:",
    ...
    HiddenChannelLockIcon: ErrorBoundary.wrap(() => (
        <Tooltip text="Hidden Channel">
            {({ onMouseLeave, onMouseEnter }) => (
                <svg
                    onMouseLeave={onMouseLeave}
