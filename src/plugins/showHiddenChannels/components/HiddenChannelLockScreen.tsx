    ...
    return (
        <div className={classes(ChatScrollClasses.auto, ChatScrollClasses.customTheme, ChatScrollClasses.managedReactiveScroller)}>
            <div className={cl("container")}>
                ...
                {lastPinTimestamp &&
                    <BaseText size="md" weight="normal">Last message pin: <Timestamp timestamp={new Date(lastPinTimestamp)} /></BaseText>
                }
                {(rateLimitPerUser ?? 0) > 0 &&
                    <BaseText size="md" weight="normal">Slowmode: {formatDuration(rateLimitPerUser!, "seconds")}</BaseText>
                }
                ...
                {((channel.isGuildVoice() || channel.isGuildStageVoice()) && bitrate != null) &&
                    <BaseText size="md" weight="normal">Bitrate: {bitrate} bits</BaseText>
                }
                {rtcRegion !== undefined &&
                    <BaseText size="md" weight="normal">Region: {rtcRegion ?? "Automatic"}</BaseText>
                }
                {(channel.isGuildVoice() || channel.isGuildStageVoice()) &&
                    <BaseText size="md" weight="normal">Video quality mode: {VideoQualityModesToNames[videoQualityMode ?? VideoQualityModes.AUTO]}</BaseText>
                }
                ...
                {defaultForumLayout != null &&
                    <BaseText size="md" weight="normal">Default layout: {ForumLayoutTypesToNames[defaultForumLayout]}</BaseText>
                }
                {defaultSortOrder != null &&
                    <BaseText size="md" weight="normal">Default sort order: {SortOrderTypesToNames[defaultSortOrder]}</BaseText>
                }
                {defaultReactionEmoji != null &&
                    <div className={cl("default-emoji-container")}>
                        <BaseText size="md" weight="normal">Default reaction emoji:</BaseText>
                ...
                {channel.hasFlag(ChannelFlags.REQUIRE_TAG) &&
                    <BaseText size="md" weight="normal">Posts on this forum require a tag to be set.</BaseText>
                }
