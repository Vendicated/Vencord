...
(async () => {
    ...
    if (!video || !playerContainer || !chaptersContainer) return;
    ...
    const api = `https://sponsor.ajay.app/api/skipSegments?videoID=${videoId}&${categories.map(c => "category=" + c).join("&")}`;
    ...
    try {
        ...
        else video.addEventListener("loadedmetadata", refresh, { once: true });
        ...
        video.addEventListener("timeupdate", () => {
            ...
            for (const seg of segments) {
                ...
                if (now >= start && now < end && !ignoredSegments.has(start.toString())) {
                    ...
                    setTimeout(() => { notice.style.opacity = "0"; notice.style.pointerEvents = "none"; }, 5000);
                    break;
                }
            }
        });
    } catch (e) { console.error("SponsorBlock Error:", e); }
})();
