...
export default definePlugin({
    ...
    renderIcon: ({ upload, channelId, draftType }: { upload: AnonUpload; draftType: any; channelId: any; }) => <ActionBarIcon
        tooltip="Anonymise file name"
        onClick={() => {
            upload.anonymise = !(upload.anonymise ?? settings.store.anonymiseByDefault);
            UploadDraft.update(channelId, upload.id, draftType, {}); // dummy update so component rerenders
        }}
    >
        {(upload.anonymise ?? settings.store.anonymiseByDefault) !== false
