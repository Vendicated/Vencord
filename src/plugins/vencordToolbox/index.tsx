...
function TallycordPopout(onClose: () => void) {
    const { useQuickCss } = useSettings(["useQuickCss"]);

    const pluginEntries = [] as ReactNode[];

    for (const plugin of Object.values(Tallycord.Plugins.plugins)) {
        if (plugin.toolboxActions && Tallycord.Plugins.isPluginEnabled(plugin.name)) {
            pluginEntries.push(
                <Menu.MenuGroup
                    label={plugin.name}
...
function TallycordPopoutIcon(isShown: boolean) {
    return (
...
function TallycordPopoutButton() {
    ...
    return (
        <Popout
            ...
            renderPopout={() => TallycordPopout(() => setShow(false))}
        >
            {(_, { isShown }) => (
                <HeaderBarIcon
                    className="vc-toolbox-btn"
                    onClick={() => setShow(v => !v)}
                    tooltip={isShown ? null : "Tallycord Toolbox"}
                    icon={() => TallycordPopoutIcon(isShown)}
                    selected={isShown}
                />
            )}
        </Popout>
...
export default definePlugin({
    name: "TallycordToolbox",
    description: "Adds a button next to the inbox button in the channel header that houses Tallycord quick actions",
