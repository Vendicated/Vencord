...
import { AlertActionCreators, Button, FluxDispatcher, Forms, GuildStore, MarkupUtils, RouterUtils, Text, Tooltip, useEffect, UserActionCreators, UserStore, useState } from "@webpack/common";

import { type Decoration, getPresets, type Preset } from "../../lib/api";
...
function SectionHeader({ section }: { section: Section; }) {
    ...
    const [authors, setAuthors] = useState<UserRecord[]>([]);
    ...
    useEffect(() => {
        (async () => {
            ...
            for (const authorId of section.authorIds) {
                const author = UserStore.getUser(authorId) ?? (await UserActionCreators.getUser(authorId))!;
        ...
        >
                                            ...
                                            >
                                                {tooltipProps => (
                                                    <DecorationGridCreate
                                                        {...tooltipProps}
                                                        className={cl("change-decoration-modal-decoration")}
                                                        onSelect={!hasDecorationPendingReview
                                                            ? settings.store.agreedToGuidelines
                    ...
                    <div className={cl("change-decoration-modal-preview")}>
                        ...
                        {activeDecorationHasAuthor && (
                            <Text key={`createdBy-${activeSelectedDecoration.authorId}`}>
                                Created by {MarkupUtils.parse(`<@${activeSelectedDecoration.authorId}>`)}
                            </Text>
                        )}
                    </div>
                </ErrorBoundary>
            ...
            <ModalFooter className={classes(cl("change-decoration-modal-footer", cl("modal-footer")))}>
                <div className={cl("change-decoration-modal-footer-btn-container")}>
                    <Button
                        onClick={() => {
                            selectDecoration(tryingDecoration!);
                            props.onClose();
                        }}
                            ...
                            <Button
                                ...
                                onClick={async () => {
                                    if (!GuildStore.getGuild(GUILD_ID)) {
                                        ...
                                        props.onClose();
                                        FluxDispatcher.dispatch({ type: "LAYER_POP_ALL" });
                                        RouterUtils.transitionToGuild(GUILD_ID);
                                    }
                                }}
                                color={Button.Colors.PRIMARY}
                                look={Button.Looks.LINK}
