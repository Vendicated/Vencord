...
function ToggleActivityComponent(activity: IgnoredActivity, isPlaying = false) {
    if (settings.store.ignoredActivities.find(act => act.id === activity.id)) return ToggleIconOff(activity, "var(--status-danger)");
...
function handleActivityToggle(e: React.MouseEvent<HTMLButtonElement, MouseEvent>, activity: IgnoredActivity, forceUpdateButton: () => void) {
    ...
    if (ignoredActivityIndex === -1) settings.store.ignoredActivities = settings.store.ignoredActivities.concat(activity);
    else settings.store.ignoredActivities = settings.store.ignoredActivities.filter((_, index) => index !== ignoredActivityIndex);
...
export default definePlugin({
    ...
    isActivityNotIgnored(props: { type: number; application_id?: string; name?: string; }) {
        if (props.type === 0 || props.type === 3) {
            if (props.application_id != null) return !settings.store.ignoredActivities.some(activity => activity.id === props.application_id);
            else {
                const exePath = RunningGameStore.getRunningGames().find(game => game.name === props.name)?.exePath;
                if (exePath) return !settings.store.ignoredActivities.some(activity => activity.id === exePath);
            }
        }
        return true;
    },
