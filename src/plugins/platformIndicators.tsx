...
function Icon(path: string, opts?: { viewBox?: string; size?: { width: number, height: number; }; }) {
    return ({ color, tooltip }: { color: string; tooltip: string; }) => (
        <Tooltip text={tooltip} >
            {(tooltipProps: any) => (
                <svg
                    {...tooltipProps}
                    height={opts?.size?.height ?? 20}
                    width={opts?.size?.width ?? 20}
