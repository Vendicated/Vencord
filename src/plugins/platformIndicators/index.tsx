...
const PlatformIcon = ({ platform, status, small }: { platform: ClientType, status: StatusType; small: boolean; }) => {
    ...
    const Icon = Icons[platform] ?? Icons[ClientType.DESKTOP];
...
function ensureOwnStatus(user: UserRecord) {
    if (user.id === UserStore.getCurrentUser()!.id) {
        ...
        const sortedSessions = Object.values(sessions).sort(({ status: a }, { status: b }) => {
            ...
            if (a === StatusType.IDLE) return 1;
            if (b === StatusType.IDLE) return -1;
            return 0;
        });
...
const indicatorLocations = {
    list: {
        ...
        onEnable() {
            addDecorator("platform-indicator", ({ user }) => (
                <ErrorBoundary noop>
    ...
    messages: {
        ...
        onEnable() {
            addDecoration("platform-indicator", ({ message }) => (
                <ErrorBoundary noop>
                    <PlatformIndicator user={message.author} wantTopMargin={true} />
                </ErrorBoundary>
            ));
        },
...
export default definePlugin({
    ...
    options: {
        ...Object.fromEntries(
            Object.entries(indicatorLocations).map(([key, value]) => [key, {
