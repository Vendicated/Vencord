...
import { PlayerState, Repeat, YoutubeMusicStore } from "./YoutubeMusicStore";

const cl = (className: string) => `vc-youtube-music-${className}`;
...
function CopyContextMenu({ name, path }: { name: string; path: string; }) {
    const copyId = `youtube-music-copy-${name}`;
    const openId = `youtube-music-open-${name}`;

    return (
        <Menu.Menu
            navId={`youtube-music-${name}-menu`}
...
function AlbumContextMenu({ track }: { track: PlayerState["song"]; }) {
    ...
    return (
        <Menu.Menu
            navId="youtube-music-album-menu"
            onClose={() => FluxDispatcher.dispatch({ type: "CONTEXT_MENU_CLOSE" })}
        ...
        >
            ...
            <Menu.MenuControlItem
                id="youtube-music-volume"
                key="youtube-music-volume"
                label="Volume"
                control={(props, ref) => (
                    <Menu.MenuSliderControl
                        {...props}
...
export function Player() {
    ...
    const exportTrackImageStyle = {
        "--vc-youtube-music-track-image": `url(${track?.imageSrc || ""})`,
